...
// rtfce_top.v
//
// RTFCE optimized top-level: 3x monitor_ctx_rtfce (timer/config only,
// no classifier) + ONE shared classify_arbiter+shared_classifier pair
// + result_arbiter (reused unchanged from baseline, per D11).
//
// NOT tt_um-prefixed -- this is a simulation/comparison vehicle only.
// Once baseline vs RTFCE comparison is complete and a design is
// selected for tapeout, the winner gets renamed to tt_um_rtfce and
// replaces project.v (see roadmap step 30 discussion).
//
// Pin interface is IDENTICAL to project.v (baseline) -- this is what
...
module rtfce_top (
    ...
    // ---- decode control pins (identical to project.v) ----
    wire        strobe      = ui_in[0];
    ...
    // ---- per-context arbiter-interface wires ----
    ...
    // ---- the ONE shared classify path (the actual optimization) ----
    ...
    // ---- result_arbiter reused UNCHANGED (D11) ----
    // grantX is used directly as result_valid_inX since only one
    // grant is ever high per cycle -- this routes the single
    // classified result to the correct per-context input line.
    ...
    // ---- global status counters (identical logic to project.v) ----
    ...
    // ---- config readback mux (identical to project.v) ----
    ...
    // ---- uo_out result/status bus (identical layout to project.v) ----
