# Top-level pin-driven test for tt_um_rtfce (baseline)
# Drives the actual TT pin interface per spec v1.1 sections 3/4/5.
...
async def test_config_readback_via_pins(dut):
    ...
    assert byte0 == expected_byte0, f"byte0 mismatch: got {byte0}, expected {expected_byte0}"

    byte1 = await cfg_read(dut, ctx=1, byte_sel=1)
    expected_byte1 = (1 << 7) | 12
    assert byte1 == expected_byte1, f"byte1 mismatch: got {byte1}, expected {expected_byte1}"


@cocotb.test()
async def test_two_way_arbitration_via_pins(dut):
    """Ctx0 and Ctx1 share the same start/end event codes so a single
    broadcast pulse starts/ends both simultaneously -- proves the
    arbiter is correctly wired at the top level, not just standalone."""
    await start_clock(dut)
    await reset_dut(dut)
